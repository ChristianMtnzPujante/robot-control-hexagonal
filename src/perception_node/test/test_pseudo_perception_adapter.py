...
def test_report_obstacle_accumulates_into_the_scene():
    ...
    adapter.report_obstacle(obstacle)

    assert adapter.get_scene().obstacles == [obstacle]
...
def test_multiple_reports_accumulate_without_losing_previous_ones():
    ...
    adapter.report_obstacle(first)
    adapter.report_obstacle(second)
    adapter.report_object("objetivo", Point(0.0, 0.0, 1.0))

    scene = adapter.get_scene()
    assert scene.obstacles == [first, second]
...
def test_get_scene_reflects_the_latest_state_each_call():
    ...
    adapter.report_obstacle(SphereObstacle(center=Point(0.0, 0.0, 0.0), radius=0.05))
